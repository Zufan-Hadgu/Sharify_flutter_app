...
import '../presentation/pages/admin/admin_lend_form_screen.dart';
import '../presentation/pages/admin/admin_lending_screen.dart';
import '../presentation/pages/admin/edit_item_screen.dart';
import '../presentation/pages/admin/lending_screen.dart';
import '../presentation/pages/auth/landing_page.dart';
import '../presentation/pages/auth/login_page.dart';
import '../presentation/pages/auth/register.dart';
...
final goRouterProvider = Provider((ref) => GoRouter(
  ...
  routes: [
    ...
    // Item Detail Route with ID Parameter
    GoRoute(
      path: "/item-detail",
    ...
    // Admin Lending Screen
    GoRoute(
      path: "/lending",
      builder: (context, state) => AdminLendingScreen(
        onAddItemClick: () {
          context.go("/add_item"); // ✅ Navigate to the item addition form
        },
      ),
    ),

    // Admin Lending Form for Item Addition
    GoRoute(
      path: "/add_item",
      builder: (context, state) => AdminLendingForm(
        onSubmit: () => print("✅ Item submitted!"),
    ...
    // ✅ Edit Item Screen Route
    GoRoute(
      path: "/edit_item/:id", // ✅ Dynamic route with item ID
      builder: (context, state) {
        final itemId = state.pathParameters['id'] ?? "";
        return EditItemScreen(itemId: itemId); // ✅ Pass ID to EditItemScreen
      },
    ),
  ],
));